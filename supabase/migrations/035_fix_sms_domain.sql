-- 035_fix_sms_domain.sql
-- The Spec 10 notification messages (028) link recipients to "locumii.ng", which is
-- not a registered domain — the live site is www.locumii.com (the apex locumii.com
-- 301s to it). Every SMS and in-app notification therefore pointed users at a dead
-- link. This recreates the five notification trigger functions with the corrected
-- domain; only the literal "locumii.ng" -> "locumii.com" changes. The existing
-- triggers reference these functions by name, so replacing the bodies is sufficient
-- (no trigger/grant changes needed).
