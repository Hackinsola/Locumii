...
function Availability() {
  ...
  async function handleSave() {
    ...
    // Validate that all enabled days have valid time windows (start < end)
    for (const day of WEEK_DAYS) {
      const window = schedule[day.key];
      if (window.on && window.start >= window.end) {
        setError(`${day.label}: start time must be before end time.`);
        return;
      }
    }
