...
- [x] **First admin seeded.** `abdulhafizakinsola@gmail.com` created directly in the live DB (per `architecture.md` — admins are not self-registerable): `auth.users` row with a bcrypt password + confirmed email + `email` identity, `public.users` row `role='admin'`/`status='active'`, and `app_metadata.role='admin'` for RLS. Seeded with a temporary password (not committed) to be changed later. **Not** captured as a migration file (one-off, contains a credential).
