...
1. **Professional onboarding** — Profile creation form (writes the `professional_profiles` row that auth currently reads `is_verified` from), document upload to the private `credentials` bucket, credential submission to the pending queue.
