...
- **Auth flow — Steps 1–2 of 5 done.**
  ...
  - **Step 3 — useAuth hook.** `src/hooks/useAuth.js`, two named exports: `useAuthListener()` (mount once at the app root — bootstraps the session via `getSession`, subscribes to `onAuthStateChange`, and keeps the store synced, fetching `is_verified` from the role-appropriate profile table) and `useAuth()` (actions `register`/`login`/`logout`/`resetPassword` + current auth state + `loading`/`error`). `register` passes `{ role, phone }` in `signUp` metadata for the trigger. Hooks never navigate — each action returns `{ error }`; explicit try/catch on every await. Lint clean.
  - **Remaining steps (awaiting go-ahead):** (4) `src/pages/auth/{Register,Login,ForgotPassword}.jsx` (plus a `/auth/reset` page for the reset link target), (5) React Router + role-based route guards (and mount `useAuthListener` at the root). `react-router-dom` and `zustand` installed.
