...
1. **First admin user** — Seed an `admin` (a `users` row with `role='admin'` + matching `auth.users`, set directly in the DB per `architecture.md`) so the credential-review panel can be built and tested. Open question: who is the first admin.
