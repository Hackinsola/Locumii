...
- **Auth flow — Steps 1–2 of 5 done.**
  - **Step 1 — registration trigger.** `supabase/migrations/014_handle_new_user.sql`: `SECURITY DEFINER` trigger `on_auth_user_created` (AFTER INSERT on `auth.users`) that creates the `public.users` row (`status='pending'`) and writes `app_metadata.role` into `auth.users.raw_app_meta_data` (the JWT claim every RLS policy reads). Role is read from `raw_user_meta_data.role` (client passes it via `signUp({ options: { data: { role, phone } } })`); only `professional`/`facility` are accepted, `admin` is rejected so privilege cannot be self-assigned. **Verified:** valid pro sign-up creates the row + claim; an `admin` sign-up is rejected with no row leaked.
  - **Step 2 — auth store.** `src/store/authStore.js`: Zustand slice holding `userId`, `role`, `isVerified`, `session`, plus `isInitialized` (false until the first session check, so guards don't flash login on refresh). Actions: `setAuth(session, isVerified)`, `setVerified`, `clearAuth`. Pure state container — no Supabase calls, no persistence (the Supabase client owns token storage); reset on logout. Lint clean.
  - **Step 3 — useAuth hook.** `src/hooks/useAuth.js`, two named exports: `useAuthListener()` (mount once at the app root — bootstraps the session via `getSession`, subscribes to `onAuthStateChange`, and keeps the store synced, fetching `is_verified` from the role-appropriate profile table) and `useAuth()` (actions `register`/`login`/`logout`/`resetPassword`/`updatePassword` + current auth state + `loading`/`error`). `register` passes `{ role, phone }` in `signUp` metadata for the trigger. Hooks never navigate — each action returns `{ error }`; explicit try/catch on every await. Lint clean.
  - **Step 4 — auth pages.** `src/pages/auth/`: `Register.jsx` (role toggle → email/phone/password/confirm, client validation, calls `register`), `Login.jsx` (email/password, generic error to avoid account enumeration), `ForgotPassword.jsx` (email → `resetPassword`, always shows the same confirmation), `Reset.jsx` (new-password form → `updatePassword`, the reset-link target). Built on the shadcn `ui/` primitives (Button/Card/Input) + their wired neutral tokens; `text-primary` links, `text-destructive` errors, no hardcoded hex, no `text-red-*`. Default-exported page components; single named `handleChange`/`validate`/`handleSubmit` handlers (no inline logic). Pages don't call Supabase directly — only via `useAuth`. **Note:** these use `react-router-dom` `Link`/`useNavigate`, so they render only once the router is mounted (step 5). `updatePassword` was added to `useAuth` to serve `Reset.jsx`. Lint clean.
  - **Remaining step (awaiting go-ahead):** (5) React Router setup + role-based protected route guards (`/professional/*`, `/facility/*`, `/admin/*`, `/auth/*`), mount `useAuthListener` at the root, and wire `/` to redirect by auth state/role. This replaces the current `App.jsx` editor-shell placeholder.
